...
認識のズレを起こさないように電話で数回連絡を取り、画面遷移図の書き直しを3回行いました。
...
#### 3、製品名と図面番号が紐づいた実装をしたこと
製品に対して図面番号が決まっており、リピート注文が多いということから、過去に注文した製品を選ぶと図面番号が表示されるように実装しました。
フォームのセレクトタグ検索ができるflexselectを使用し、jQueryのコードに製品名と図面番号が紐づくようにコードを追加しました。
該当するコードの検索や実装したいコードを書くことに時間がかかりました。

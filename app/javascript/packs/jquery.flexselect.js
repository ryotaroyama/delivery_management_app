...
;(function ($) {
  ...
  $.extend($.flexselect.prototype, {
    ...
    wire: function () {
      ...
      this.input.blur(function () {
        if (!self.dropdownMouseover) {
          self.hide()
          if (self.settings.allowMismatchBlank && $.trim($(this).val()) == '') self.setValue('')
    ...
    adjustMaxHeight: function () {
      ...
      this.dropdown.css(
        'max-height',
        top > maxTop ? Math.max(0, maxTop - top + this.dropdown.innerHeight()) + 'px' : ''
      )
    },
      ...
      if (selected && !selected.disabled) {
        ...
        if (this.input.attr('id') === 'order_name_flexselect') {
          fetch(`/drawing_numbers/search_number?product_name=${selected.value}`)
            ...
            .then((data) => {
              document.getElementById(`drawing_number_name`).value = data.drawing_number
            })
        }
